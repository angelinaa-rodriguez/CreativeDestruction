...
params0.phi    = fitted(2);
params0.beta   = fitted(3);
params0.kappa  = fitted(4);    
params0.lambda = fitted(5);
params0.delta  = fitted(6);    
params0.gamma  = fitted(7);
params0.rho    = fitted(8);
...
for i=1:M
    ...
    if abs(v) < 1e-12
        ...
        switch pname
            case 'alpha'; idx_est = 1;
            case 'phi';   idx_est = 2;
            case 'beta';  idx_est = 3;
            case 'kappa'; idx_est = 4;
            case 'lambda';idx_est = 5;
            case 'delta'; idx_est = 6;
            case 'gamma'; idx_est = 7;
            case 'rho';   idx_est = 8;
        end
        lb(i) = est_lb(idx_est);
        ub(i) = est_ub(idx_est);
...
% remove arameter columns with basically zero variance
...
save('sensitivity_results.mat','out');

writetable(out.paramSamples, 'param_samples.csv');
writematrix([E_out, W_out], 'outputs.csv');
TBL = table(paramsUsed', prccE, prccW, 'VariableNames',{'Parameter','PRCC_E','PRCC_W'});
writetable(TBL,'prcc_summary.csv');
